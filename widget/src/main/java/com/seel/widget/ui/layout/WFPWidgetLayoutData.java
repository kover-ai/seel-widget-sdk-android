package com.seel.widget.ui.layout;

import com.seel.widget.models.QuotesResponse;

/**
 * Data model passed to widget layout providers for rendering.
 * Aligned with iOS WFPWidgetLayoutData.
 */
public class WFPWidgetLayoutData {
    private final QuotesResponse quoteResponse;
    private final boolean loading;
    private final ToggleStyle toggleStyle;
    private final boolean toggleIsOn;
    private final Integer normalBackgroundColor;
    private final Integer selectedBackgroundColor;
    private final Integer disabledBackgroundColor;
    private final boolean showDisclaimer;

    public WFPWidgetLayoutData(QuotesResponse quoteResponse, boolean loading,
                               ToggleStyle toggleStyle, boolean toggleIsOn,
                               Integer normalBackgroundColor, Integer selectedBackgroundColor,
                               Integer disabledBackgroundColor, boolean showDisclaimer) {
        this.quoteResponse = quoteResponse;
        this.loading = loading;
        this.toggleStyle = toggleStyle;
        this.toggleIsOn = toggleIsOn;
        this.normalBackgroundColor = normalBackgroundColor;
        this.selectedBackgroundColor = selectedBackgroundColor;
        this.disabledBackgroundColor = disabledBackgroundColor;
        this.showDisclaimer = showDisclaimer;
    }

    public QuotesResponse getQuoteResponse() { return quoteResponse; }
    public boolean isLoading() { return loading; }
    public ToggleStyle getToggleStyle() { return toggleStyle; }
    public boolean isToggleOn() { return toggleIsOn; }
    /** Returns null if not explicitly set by the SDK user. */
    public Integer getNormalBackgroundColor() { return normalBackgroundColor; }
    /** Returns null if not explicitly set by the SDK user. */
    public Integer getSelectedBackgroundColor() { return selectedBackgroundColor; }
    /** Returns null if not explicitly set by the SDK user. */
    public Integer getDisabledBackgroundColor() { return disabledBackgroundColor; }
    public boolean isShowDisclaimer() { return showDisclaimer; }
}
